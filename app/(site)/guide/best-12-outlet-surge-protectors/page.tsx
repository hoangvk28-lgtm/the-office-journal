...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-12-outlet-surge-protectors";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-12-outlet-surge-protectors",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-12-outlet-surge-protectors` },
    about: [{"@type":"Thing","name":"Best 12-Outlet Surge Protectors"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 12-Outlet Surge Protectors", item: `${SITE_URL}/guide/best-12-outlet-surge-protectors` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-12-outlet-surge-protectors#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 12-Outlet Surge Protectors</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best 12-Outlet Surge Protectors" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>12 rated outlets is where the usable-outlet-count gap becomes most severe in absolute terms, a high-density outlet layout with 12 slots in a compact strip body has even less spacing margin per outlet than a 6-outlet unit, often leaving considerably fewer than 12 genuinely usable slots with real-world adapters.</p>
          <p>At 12 outlets, the joule rating is shared across the largest number of potential devices of any tier, a high-value home theater or full office setup sharing one joule budget across 12 devices depletes that shared budget faster with more surges than a lower-outlet-count unit protecting fewer devices.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 12-Outlet Surge Protector</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 12-Outlet Surge Protectors</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 12-Outlet Surge Protectors</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=12+outlet+surge+protector&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
