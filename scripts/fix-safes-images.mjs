...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/safes_pool.json"), "utf8"));
const poolArrRaw = [];
...
const poolArr = Object.values(byAsin);

function bucket(tagSubstr) {
  return poolArr.filter((p) => p.tags.some((t) => t.includes(tagSubstr)));
}
const buckets = {
  small: bucket("small fireproof"),
  large: bucket("large fireproof"),
  digital: bucket("digital lock"),
  biometric: bucket("biometric"),
  waterproof: bucket("waterproof safe combination"),
  document: bucket("document safe cash"),
  oneCuFt: bucket("1 cubic feet"),
  twoCuFt: bucket("2 cubic feet"),
  fourCuFt: bucket("4 cubic feet"),
  budget: bucket("budget fireproof"),
  honeywell: bucket("Honeywell").filter((p) => /honeywell/i.test(p.title)),
  tigerking: bucket("TigerKing").filter((p) => /tigerking/i.test(p.title)),
};

function categorize(filename) {
  const f = filename.toLowerCase();
  if (f.includes("honeywell")) return buckets.honeywell.length ? buckets.honeywell : buckets.small;
  if (f.includes("tigerking")) return buckets.tigerking.length ? buckets.tigerking : buckets.large;
  if (f.includes("combination")) return buckets.waterproof;
  if (f.includes("for-cash")) return buckets.document;
  if (f.includes("large")) return buckets.large;
  if (f.includes("small")) return buckets.small;
  if (f.includes("0-18") || f.includes("1-cu") || f.includes("1-cubic") || f.includes("1-2-cu") || f.includes("1-2-cubic") || f.includes("1-5-cu") || f.includes("1-5-cubic")) return buckets.oneCuFt;
  if (f.includes("2-0-cubic") || f.includes("2-cu") || f.includes("2-cubic") || f.includes("2-hour")) return buckets.twoCuFt;
  if (f.includes("4-cu") || f.includes("4-cubic") || f.includes("5-cu")) return buckets.fourCuFt;
  if (f.includes("under-50") || f.includes("under-500")) return buckets.budget;
  if (f.includes("under-1000") || f.includes("under-1500")) return [...buckets.digital, ...buckets.large];
  return buckets.digital.length ? buckets.digital : buckets.small;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?fireproof-safes?-?/g, " ").replace(/-?safes?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose fireproof safe";
}

const PRODUCT_NOUN_PLURAL = "safes";
const PRODUCT_CATEGORY_USE = "daily home or office security use";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  const para3 = `Best for buyers specifically looking for ${fileTheme}, though as with any security purchase, confirm current price and stock directly on the Amazon listing before ordering.`;
  ...
  const raw = {
    ...
    imageUrl: img,
    amazonUrl: `https://www.amazon.com/dp/${asin}?tag=theofficejournal-20`,
    description: `${para1}\n\n${para2}\n\n${para3}`,
...
console.log("Safes files with broken slots:", brokenFileSet.size);
