...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-for-bedroom";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-for-bedroom` },
    about: [{"@type":"Thing","name":"TV Stand for Bedroom"},{"@type":"Thing","name":"Low-Profile TV Console"},{"@type":"Thing","name":"Bedroom Furniture"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stands for Bedroom", item: `${SITE_URL}/guide/best-tv-stand-for-bedroom` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-for-bedroom#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stands for Bedroom</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              {products.length} bedroom TV stands evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stands for Bedroom" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A bedroom TV stand has different priorities than a living room console: lower profile so the screen sits at a comfortable lying-down viewing angle, a smaller footprint for tighter rooms, and often extra storage since bedrooms rarely have a separate dresser and media unit.</p>
          <p>Before buying, measure the space between your bed and the wall the stand will sit against, and check whether a dresser-combo design actually gives you the drawer capacity you need.</p>
          <p>This guide covers {products.length} TV stands built for bedroom use, ranked by height, footprint, and storage.</p>
        </section>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-ink-secondary">
                ...
                {" "}- {products[0]?.badge.toLowerCase()} pick in this roundup for bedroom setups.
              </p>
            </div>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+for+bedroom&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
