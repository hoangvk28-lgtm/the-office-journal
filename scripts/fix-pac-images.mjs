...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/pac_pool.json"), "utf8"));
const poolArrRaw = [];
...
const isRealPAC = (p) => /portable air conditioner/i.test(p.title) && !/fan\b|evaporative|mini air conditioner/i.test(p.title);
const poolArr = Object.values(byAsin).filter(isRealPAC);

function bucket(tagSubstr) {
  return poolArr.filter((p) => p.tags.some((t) => t.includes(tagSubstr)));
}
const buckets = {
  budgetRoom: bucket("portable air conditioner room budget"),
  eightK: bucket("8000 BTU"),
  large: bucket("large room"),
};
...
function categorize(filename) {
  ...
  if (f.includes("under-400")) return under(400).length ? under(400) : buckets.budgetRoom;
  if (f.includes("under-500")) return under(500).length ? under(500) : buckets.eightK;
  if (f.includes("under-700")) return under(700).length ? under(700) : buckets.large;
  if (f.includes("under-1000")) return under(1000).length ? under(1000) : buckets.large;
  return buckets.eightK.length ? buckets.eightK : buckets.budgetRoom;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?portable-air-conditioners?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose portable air conditioner";
}

const PRODUCT_NOUN_PLURAL = "portable air conditioners";
const PRODUCT_CATEGORY_USE = "cooling a bedroom or home office without central air";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  const para3 = `Best for buyers specifically looking for ${fileTheme}, though as with any appliance purchase, confirm current price and stock directly on the Amazon listing before ordering.`;
...
console.log("Portable AC files with broken slots:", brokenFileSet.size);
