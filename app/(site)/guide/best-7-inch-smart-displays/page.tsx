...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-7-inch-smart-displays";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-7-inch-smart-displays",
...
export default async function Page() {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 7 Inch Smart Display", item: `${SITE_URL}/guide/best-7-inch-smart-displays` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-7-inch-smart-displays#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 7 Inch Smart Display</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best 7 Inch Smart Display" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Our verified research pool contains exactly one genuine 7 inch class smart display, the Google Nest Hub 2nd generation. We looked for a comparable 7 inch Echo Show or other competing model, but Amazon's Echo Show lineup jumps from the roughly 5.5 inch Echo Show 5 straight to the 8.7 inch and larger models, with no verified genuine 7 inch Echo Show in current production.</p>
          <p>Rather than round an 8 inch display down to fit this search term, we're giving the Nest Hub a thorough single-product review below, including its most distinctive trait: no built-in camera at all.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 7 Inch Smart Display</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 7 Inch Smart Display</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 7 Inch Smart Display</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=7+inch+smart+display&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
