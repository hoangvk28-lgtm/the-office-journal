...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-in-wall-small-safes";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-in-wall-small-safes",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-in-wall-small-safes` },
    about: [{"@type":"Thing","name":"Best In-Wall Small Safes: An Honest Guide"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best In-Wall Small Safes: An Honest Guide", item: `${SITE_URL}/guide/best-in-wall-small-safes` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-in-wall-small-safes#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best In-Wall Small Safes: An Honest Guide</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best In-Wall Small Safes: An Honest Guide" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>We need to be direct here: none of the 8 listings in our research pool document rough-opening dimensions, stud-bay depth, or framing-anchor instructions, the specifications a genuine in-wall safe needs for installation between wall studs. These are general mountable security safes, not purpose-built in-wall units.</p>
          <p>Rather than imply in-wall installation compatibility these listings don't confirm, we're presenting the strongest general mountable safes from our research and explaining what to actually verify before attempting to install any safe inside a wall cavity.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a In-Wall Small Safes: An Honest Guide</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These In-Wall Small Safes: An Honest Guide</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right In-Wall Small Safes: An Honest Guide</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=in-wall+small+safes&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
