...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-mini-ups-for-routers-and-cctv-cameras";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-mini-ups-for-routers-and-cctv-cameras",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-mini-ups-for-routers-and-cctv-cameras` },
    about: [{"@type":"Thing","name":"Best Mini UPS for Routers and CCTV Cameras"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Mini UPS for Routers and CCTV Cameras", item: `${SITE_URL}/guide/best-mini-ups-for-routers-and-cctv-cameras` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-mini-ups-for-routers-and-cctv-cameras#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Mini UPS for Routers and CCTV Cameras</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best Mini UPS for Routers and CCTV Cameras" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Backing up a router and a CCTV camera together is a genuinely different problem than backing up a router alone: camera load scales with count and infrared night-mode draw, and many cameras run on PoE rather than a simple DC barrel jack, which most router-only mini UPS units don't output.</p>
          <p>We prioritized units in this cluster that publish both DC and PoE output options, since that combination is what actually lets one unit cover a router and a PoE camera without a second backup device.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Mini UPS for Routers and CCTV Camera</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Mini UPS for Routers and CCTV Cameras</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Mini UPS for Routers and CCTV Cameras</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=mini+ups+for+routers+and+cctv+cameras&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
