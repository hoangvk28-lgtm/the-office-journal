...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-usb-c-vertical-mice";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-usb-c-vertical-mice",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-usb-c-vertical-mice` },
    about: [{"@type":"Thing","name":"Best USB-C Vertical Mice"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best USB-C Vertical Mice", item: `${SITE_URL}/guide/best-usb-c-vertical-mice` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-usb-c-vertical-mice#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best USB-C Vertical Mice</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best USB-C Vertical Mice" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>USB-C in this context could refer to the charging port, the wireless dongle's connector, or a wired connection method, worth clarifying which specific USB-C implementation each product uses rather than treating 'USB-C' as one undifferentiated feature.</p>
          <p>If USB-C refers to a genuinely wired connection, a USB-C-cabled mouse rather than the more common USB-A, cable quality and length considerations deserve the same scrutiny given to wired accessory research generally.</p>
          <p>USB-C dongle compatibility with modern USB-C-only laptops, avoiding the need for a separate USB-A adapter, is a genuine convenience benefit worth highlighting for buyers with newer laptops that have phased out full-size USB-A ports entirely.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a USB-C Vertical Mice</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These USB-C Vertical Mice</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right USB-C Vertical Mice</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=USB-C+vertical+mouse&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
