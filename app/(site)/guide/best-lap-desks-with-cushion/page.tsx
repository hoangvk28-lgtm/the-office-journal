...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-lap-desks-with-cushion";
...
export default async function BestLapDesksWithCushionPage() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lap-desks-with-cushion` },
    about: [
      { "@type": "Thing", name: "Cushion Lap Desk" },
      { "@type": "Thing", name: "Lap Desk" },
      { "@type": "Thing", name: "Memory Foam Accessories" },
    ],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lap Desks with Cushion", item: `${SITE_URL}/guide/best-lap-desks-with-cushion` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lap-desks-with-cushion#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lap Desks with Cushion</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              8 lap desks with cushion evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Lap Desks with Cushion" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>The cushion base on a lap desk serves two functions: it distributes the hard surface pressure away from the legs and creates a slight air gap that reduces heat transfer from the laptop. Without a cushion, a hard lap desk board puts concentrated pressure on the thighs and transfers laptop heat directly.</p>
          <p>This guide covers eight cushion lap desks ranked by Amazon buyer rating and verified review count, from standard pillow foam bases and bean bag fill to memory foam options and premium adjustable models with cushion bases.</p>
          <p>For lap desks focused on bed use, see our <Link prefetch={false} href='/guide/best-lap-desks-for-bed' className='text-brand hover:underline'>best lap desks for bed</Link> guide.</p>
        </section>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-ink-secondary">
                <strong className="text-ink">best cushion lap desk:</strong>{" "}
                <a href="#lapgear-home-office-pro" className="font-bold text-ink hover:text-brand transition-colors">LapGear Home Office Pro Lap Desk</a>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href="https://www.amazon.com/s?k=lap+desk+with+cushion&tag=theofficejournal-20" target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
