...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-long-run-paper-shredders";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-long-run-paper-shredders",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-long-run-paper-shredders` },
    about: [{"@type":"Thing","name":"Best Long-Run Paper Shredders"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Long-Run Paper Shredders", item: `${SITE_URL}/guide/best-long-run-paper-shredders` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-long-run-paper-shredders#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Long-Run Paper Shredders</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best Long-Run Paper Shredders" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A '60-minute continuous run' rating assumes consistent, light paper feeding under lab conditions. Real bulk-shredding sessions with mixed paper weights and staples will often hit the thermal limit before the rated time, an optimistic-lab-condition gap we account for by ranking based on the disclosed run time relative to bin capacity and cooldown period together, not the run-time number alone.</p>
          <p>Cooldown time after hitting the run-time limit varies significantly, some picks need 10 minutes, others 40-60, a real difference that can double the actual time needed to shred a large document purge.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Long-Run Paper Shredder</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Long-Run Paper Shredders</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Long-Run Paper Shredders</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=continuous-duty+paper+shredder&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
