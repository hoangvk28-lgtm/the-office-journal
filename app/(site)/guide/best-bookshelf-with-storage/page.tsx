...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bookshelf-with-storage";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bookshelf-with-storage",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bookshelf-with-storage` },
    about: [{"@type":"Thing","name":"Best Bookshelves with Storage"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bookshelves with Storage", item: `${SITE_URL}/guide/best-bookshelf-with-storage` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bookshelf-with-storage#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bookshelves with Storage</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Bookshelves with Storage" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A bookshelf with storage goes beyond simple open shelving, adding cubes, cabinets, or drawers so you can keep everyday clutter out of sight while still displaying your favorite books and decor on the open tiers.</p>
          <p>Below are 8 bookshelves with storage we evaluated on capacity, enclosed versus open storage ratio, and footprint, useful for living rooms, home offices, and small bedrooms where a separate dresser or media console isn't practical.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bookshelf+with+storage&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
