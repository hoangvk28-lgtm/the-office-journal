...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-hp-monitor-privacy-screens";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-hp-monitor-privacy-screens",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-hp-monitor-privacy-screens` },
    about: [{"@type":"Thing","name":"Best HP Monitor Privacy Screens"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best HP Monitor Privacy Screens", item: `${SITE_URL}/guide/best-hp-monitor-privacy-screens` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-hp-monitor-privacy-screens#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best HP Monitor Privacy Screens</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best HP Monitor Privacy Screens" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>We need to be upfront: HP does not manufacture monitor privacy filters. Every product searchable under "HP monitor privacy screen" is a third-party filter sized to fit common HP monitor dimensions, not a certified HP accessory.</p>
          <p>What actually determines fit is your exact HP monitor's active width, height, and aspect ratio, not the brand name in the filter's listing title. We matched picks to the most common HP monitor sizes (22, 24, and 27 inch, all standard 16:9) and flagged where you need to verify your specific model.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a HP Monitor Privacy Screen</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These HP Monitor Privacy Screens</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right HP Monitor Privacy Screens</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=hp+monitor+privacy+screen&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
