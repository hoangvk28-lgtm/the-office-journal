...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/studiomon_pool.json"), "utf8"));
const poolArrRaw = [];
...
// keep only genuine studio monitor speakers, exclude generic PC speakers, stands, etc.
const isRealMonitor = (p) =>
  /studio monitor/i.test(p.title) && !/stand\b/i.test(p.title);
const poolArr = Object.values(byAsin).filter(isRealMonitor);

function bucket(tagSubstr) {
  return poolArr.filter((p) => p.tags.some((t) => t.includes(tagSubstr)));
}
const buckets = {
  budget: bucket("budget studio monitor"),
  fiveInch: bucket("5 inch studio"),
  professional: bucket("professional studio"),
  powered: bucket("powered studio monitor speakers desktop"),
  premium: bucket("premium studio monitor"),
};
...
function categorize(filename) {
  ...
  const under = (n) => poolArr.filter((p) => {
    const pn = priceNum(p);
    return pn !== null && pn < n;
  });
  if (f.includes("under-150")) return under(150).length ? under(150) : buckets.budget;
  if (f.includes("under-200")) return under(200).length ? under(200) : buckets.budget;
  if (f.includes("under-300")) return under(300).length ? under(300) : buckets.budget;
  if (f.includes("under-500")) return under(500).length ? under(500) : buckets.professional;
  if (f.includes("under-1000")) return under(1000).length ? under(1000) : buckets.professional;
  if (f.includes("under-2000")) return under(2000).length ? under(2000) : buckets.premium;
  if (f.includes("under-5000")) return poolArr; // whole pool is under $5000
  return buckets.professional.length ? buckets.professional : buckets.budget;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?studio-monitors?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose studio monitor speaker";
}

const PRODUCT_NOUN_PLURAL = "studio monitors";
const PRODUCT_CATEGORY_USE = "accurate near-field audio monitoring at a desk";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  else cons.push(`At ${price} per pair, worth comparing against similarly specced ${PRODUCT_NOUN_PLURAL} before committing`);
...
console.log("Studio monitor files with broken slots:", brokenFileSet.size);
