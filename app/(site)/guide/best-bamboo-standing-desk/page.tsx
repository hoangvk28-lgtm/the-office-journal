...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bamboo-standing-desk";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bamboo-standing-desk` },
    about: [{"@type":"Thing","name":"Bamboo Standing Desk"},{"@type":"Thing","name":"FSC Certified Desk"},{"@type":"Thing","name":"Electric Height Adjustable Desk"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bamboo Standing Desks", item: `${SITE_URL}/guide/best-bamboo-standing-desk` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bamboo-standing-desk#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bamboo Standing Desks</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Bamboo Standing Desks" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A bamboo standing desk replaces MDF or laminate with FSC-certified bamboo - a surface that is harder, more moisture-resistant, and grown from one of the fastest-renewing raw materials in furniture production.</p>
          <p>This guide covers eight bamboo electric standing desks ranked by FSC certification, bamboo construction (one-piece vs laminate), motor type (single vs dual), height range, and price.</p>
          <p>For bamboo desks with higher maximum height coverage for tall users, see our <a href='/guide/best-standing-desk-for-tall-person' className='text-brand hover:underline'>best standing desks for tall people</a> guide.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bamboo+standing+desk+electric+FSC&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
