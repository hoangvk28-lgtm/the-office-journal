...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-mini-ups-for-routers-and-onus";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-mini-ups-for-routers-and-onus",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-mini-ups-for-routers-and-onus` },
    about: [{"@type":"Thing","name":"Best Mini UPS for Routers and ONUs"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Mini UPS for Routers and ONUs", item: `${SITE_URL}/guide/best-mini-ups-for-routers-and-onus` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-mini-ups-for-routers-and-onus#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Mini UPS for Routers and ONUs</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best Mini UPS for Routers and ONUs" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Router-plus-ONU is one of the most genuinely well-supported combinations in this category, several brands explicitly market products for exactly this pairing, rather than it being an edge case retrofitted onto a single-device design.</p>
          <p>We prioritized units with documented dual-rail output at the two voltages a router and ONU typically need, since that's what actually determines whether both devices stay powered rather than just one.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Mini UPS for Routers and ONU</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Mini UPS for Routers and ONUs</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Mini UPS for Routers and ONUs</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=mini+ups+for+routers+and+onus&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
