...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bookshelf-tv-stand";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bookshelf-tv-stand",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bookshelf-tv-stand` },
    about: [{"@type":"Thing","name":"Best Bookshelf TV Stands"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bookshelf TV Stands", item: `${SITE_URL}/guide/best-bookshelf-tv-stand` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bookshelf-tv-stand#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bookshelf TV Stands</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Bookshelf TV Stands" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A bookshelf TV stand combines a media console with tall, integrated shelving on one or both sides of the screen, turning what would otherwise be a narrow entertainment center into a full wall unit that also handles books, decor, and display items.</p>
          <p>Below are 6 bookshelf TV stands we evaluated on shelf capacity, cabinet configuration, and extras like built-in LED lighting, power outlets, and even electric fireplaces, all useful for maximizing wall space in a smaller living room.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bookshelf+tv+stand&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
