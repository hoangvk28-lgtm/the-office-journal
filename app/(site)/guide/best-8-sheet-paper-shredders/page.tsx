...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-8-sheet-paper-shredders";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-8-sheet-paper-shredders",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-8-sheet-paper-shredders` },
    about: [{"@type":"Thing","name":"Best 8-Sheet Paper Shredders"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 8-Sheet Paper Shredders", item: `${SITE_URL}/guide/best-8-sheet-paper-shredders` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-8-sheet-paper-shredders#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 8-Sheet Paper Shredders</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best 8-Sheet Paper Shredders" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Rated 8-sheet capacity assumes standard 20 lb bond paper tested under ideal lab conditions. Real-world capacity with letterhead, cardstock, or slightly heavier paper is often 5-6 sheets, a 25-35% reduction rarely disclosed at this entry-level capacity tier. We treat the rated 8-sheet number as an upper bound throughout this guide.</p>
          <p>This tier also typically has the shortest continuous run times in the shredder category, 2-5 minutes, and skews toward lighter security levels (P-3/P-4 cross-cut rather than higher-security micro-cut), both genuine tradeoffs of buying at this entry-level capacity.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 8-Sheet Paper Shredder</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 8-Sheet Paper Shredders</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 8-Sheet Paper Shredders</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=8-sheet+paper+shredder&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
