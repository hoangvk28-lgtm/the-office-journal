...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-lap-desks-for-bed";
...
export default async function BestLapDesksForBedPage() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lap-desks-for-bed` },
    about: [
      { "@type": "Thing", name: "Lap Desk" },
      { "@type": "Thing", name: "Bed Accessories" },
      { "@type": "Thing", name: "Laptop in Bed" },
    ],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lap Desks for Bed", item: `${SITE_URL}/guide/best-lap-desks-for-bed` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lap-desks-for-bed#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lap Desks for Bed</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Lap Desks for Bed" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Using a laptop in bed without a lap desk puts the device directly on soft bedding that blocks bottom vents and causes overheating. A lap desk provides a hard surface that elevates the laptop above the mattress, keeps vents clear, and protects legs from the heat the laptop generates.</p>
          <p>This guide covers eight lap desks for bed use ranked by Amazon buyer rating and verified review count, including cushioned flat boards, adjustable-angle designs, height-adjustable models, and foldable options for small bedroom storage.</p>
          <p>For lap desks focused on cushion base comfort, see our <Link prefetch={false} href='/guide/best-lap-desks-with-cushion' className='text-brand hover:underline'>best lap desks with cushion</Link> guide.</p>
        </section>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-ink-secondary">
                <strong className="text-ink">best lap desk for bed:</strong>{" "}
                <a href="#saiji-xl" className="font-bold text-ink hover:text-brand transition-colors">SAIJI X-Large Adjustable Lap Desk</a>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href="https://www.amazon.com/s?k=lap+desk+for+bed&tag=theofficejournal-20" target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
