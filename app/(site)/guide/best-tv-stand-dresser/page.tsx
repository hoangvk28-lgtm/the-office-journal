...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-dresser";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-tv-stand-dresser",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-dresser` },
    about: [{"@type":"Thing","name":"Best TV Stand Dresser Combos"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stand Dresser Combos", item: `${SITE_URL}/guide/best-tv-stand-dresser` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-dresser#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stand Dresser Combos</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stand Dresser Combos" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A TV stand dresser combines a chest of drawers with a wide, low-profile top built to hold a television, which makes it a practical fit for bedrooms and studio apartments where floor space for two separate pieces simply doesn't exist. The best versions pair real drawer storage for clothing or linens with a sturdy top rated for common TV sizes, and many now add LED lighting and built-in charging outlets that a standalone dresser or stand won't offer.</p>
          <p>Choosing one over a separate TV stand and dresser makes the most sense when floor space is tight or a single piece needs to anchor a small room, but it's worth going in with realistic expectations about drawer depth and weight distribution, both of which we cover in the buying criteria below.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+dresser&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
