...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-2-bay-nas-for-home-offices";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-2-bay-nas-for-home-offices",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-2-bay-nas-for-home-offices` },
    about: [{"@type":"Thing","name":"Best 2-Bay NAS for Home Offices"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 2-Bay NAS for Home Offices", item: `${SITE_URL}/guide/best-2-bay-nas-for-home-offices` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-2-bay-nas-for-home-offices#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 2-Bay NAS for Home Offices</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best 2-Bay NAS for Home Offices" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A 2-bay NAS is the entry point for real home office network storage, but the sticker price on the enclosure is only part of the real cost. Two drives, and in some cases a faster switch to actually see the network speed gains, usually add several hundred dollars before the unit is doing anything useful.</p>
          <p>Below are 8 diskless 2-bay NAS units we evaluated on true all-in cost, RAM upgradeability, and how honestly each brand handles RAID 1's storage math, ranging from a $199 entry-level Asustor box to a $519 expandable Synology unit built for small business use.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 2-Bay NAS for Home Office</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 2-Bay NAS for Home Offices</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 2-Bay NAS for Home Offices</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=2+bay+nas+home+office&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
