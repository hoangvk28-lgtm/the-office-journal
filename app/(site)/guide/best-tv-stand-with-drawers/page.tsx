...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-with-drawers";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-with-drawers` },
    about: [{"@type":"Thing","name":"TV Stand with Drawers"},{"@type":"Thing","name":"Entertainment Center"},{"@type":"Thing","name":"Media Console Storage"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stands with Drawers", item: `${SITE_URL}/guide/best-tv-stand-with-drawers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-with-drawers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stands with Drawers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stands with Drawers" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A TV stand with drawers gives you enclosed, quick-access storage for remotes, cables, and controllers without the swing clearance a cabinet door needs.</p>
          <p>This guide covers seven TV stands ranked specifically on real pull-out drawer construction, glide quality, and drawer interior depth, not just open shelves or cabinet doors marketed loosely as "storage."</p>
          <p>For a broader mix of cabinets, shelves, and drawers, see our <Link prefetch={false} href='/guide/best-tv-stand-with-storage' className='text-brand hover:underline'>best TV stands with storage</Link> guide instead.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+with+drawers&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
