...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-10-inch-smart-displays";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-10-inch-smart-displays",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-10-inch-smart-displays` },
    about: [{"@type":"Thing","name":"Best 10 Inch Smart Display"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 10 Inch Smart Display", item: `${SITE_URL}/guide/best-10-inch-smart-displays` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-10-inch-smart-displays#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 10 Inch Smart Display</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best 10 Inch Smart Display" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>The Amazon Echo Show 10, at roughly 10.1 inches, is the only genuinely 10 inch class smart display in our verified research pool. It's a somewhat thin category on its own merits, most competing displays sit either around 7 to 8 inches or jump up to 15 inches, with relatively few landing specifically in the 10 inch range.</p>
          <p>Rather than round up a smaller display or round down a 15 inch one to pad this list, we're giving the Echo Show 10 a thorough single-product review, since its motorized rotating base is genuinely the most distinctive feature in this entire product category regardless of size class.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 10 Inch Smart Display</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 10 Inch Smart Display</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 10 Inch Smart Display</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=10+inch+smart+display&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
