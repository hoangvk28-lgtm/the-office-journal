...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/laser_pool.json"), "utf8"));
const poolArrRaw = [];
...
const buckets = {
  uv10w: bucket("10W UV laser"),
  fiber: bucket("fiber laser engraver"),
  mini: bucket("mini laser engraver"),
  uvGlass: bucket("UV laser engraver glass"),
  rotary: bucket("rotary attachment roller"),
  wainlux: bucket("Wainlux laser").filter((p) => /wainlux/i.test(p.title)),
  diode: bucket("diode laser engraver"),
};

function categorize(filename) {
  const f = filename.toLowerCase();
  if (f.includes("wainlux")) return buckets.wainlux.length ? buckets.wainlux : buckets.mini;
  if (f.includes("rotary-attachment")) return buckets.rotary;
  if (f.includes("10w-uv")) return buckets.uv10w;
  if (f.includes("fiber")) return buckets.fiber;
  if (f.includes("mini")) return buckets.mini;
  if (f.includes("uv-laser-engravers-for-glass") || f.includes("uv-laser-engravers")) return buckets.uvGlass.length ? buckets.uvGlass : buckets.uv10w;
  return buckets.diode.length ? buckets.diode : buckets.mini;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?laser-engravers?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose laser engraver";
}

const PRODUCT_NOUN_PLURAL = "laser engravers";
const PRODUCT_CATEGORY_USE = "hobbyist and small-business engraving projects";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  const para3 = `Best for buyers specifically looking for ${fileTheme}, though as with any equipment purchase, confirm current price and stock directly on the Amazon listing before ordering.`;
...
console.log("Laser engraver files with broken slots:", brokenFileSet.size);
