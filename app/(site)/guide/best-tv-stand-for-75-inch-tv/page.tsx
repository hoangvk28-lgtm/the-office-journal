...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-for-75-inch-tv";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-for-75-inch-tv` },
    about: [{"@type":"Thing","name":"TV Stand for 75 Inch TV"},{"@type":"Thing","name":"Large TV Console"},{"@type":"Thing","name":"Entertainment Center"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stands for 75 Inch TVs", item: `${SITE_URL}/guide/best-tv-stand-for-75-inch-tv` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-for-75-inch-tv#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stands for 75 Inch TVs</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              {products.length} large-screen TV stands evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stands for 75 Inch TVs" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A 75-inch TV puts more demand on a stand than smaller screens: the base footprint is wider, the panel is heavier, and a stand rated for a 55-inch TV will often be too narrow or under-rated to safely hold it.</p>
          <p>Before buying, check the stand&apos;s width against your TV&apos;s actual base foot spread (not just diagonal size), and confirm the weight capacity has margin for the TV plus a soundbar or media console.</p>
          <p>This guide covers {products.length} TV stands built for 70 to 80 inch screens, ranked by width, weight capacity, and stability.</p>
        </section>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-ink-secondary">
                ...
                {" "}- {products[0]?.badge.toLowerCase()} pick in this roundup for large-screen stability.
              </p>
            </div>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+for+75+inch+tv&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
