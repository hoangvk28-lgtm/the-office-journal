...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-bookshelf-cabinet";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bookshelf-cabinet",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bookshelf-cabinet` },
    about: [{"@type":"Thing","name":"Best Bookshelf Cabinets"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bookshelf Cabinets", item: `${SITE_URL}/guide/best-bookshelf-cabinet` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bookshelf-cabinet#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bookshelf Cabinets</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Bookshelf Cabinets" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A bookshelf cabinet gives you the best of both worlds, open shelving for the books and decor you want on display, plus enclosed doors or drawers for everything you'd rather keep out of sight.</p>
          <p>Below are 8 bookshelf cabinets we evaluated on capacity, construction, and standout features, ranging from farmhouse-style door combos under $100 to glass display cabinets with automatic LED lighting.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bookshelf+cabinet&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
