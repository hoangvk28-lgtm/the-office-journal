...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-on-wheels";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-on-wheels` },
    about: [{"@type":"Thing","name":"TV Stand on Wheels"},{"@type":"Thing","name":"Rolling TV Cart"},{"@type":"Thing","name":"Mobile TV Stand"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stands on Wheels", item: `${SITE_URL}/guide/best-tv-stand-on-wheels` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-on-wheels#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stands on Wheels</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              {products.length} carts evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stands on Wheels" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A TV stand on wheels lets you move your screen between rooms, reposition it for glare-free viewing, or roll it away entirely, something a fixed console or wall mount cannot do.</p>
          <p>This guide covers eight rolling TV carts ranked by weight capacity, height range, wheel locking design, and base stability, from budget picks under $100 to a heavy-duty 250 lb cart built for large-screen displays.</p>
          <p>For a fixed TV stand instead, see our <Link prefetch={false} href="/guide/best-tv-stand-for-55-inch-tv" className="text-brand hover:underline">best TV stands for 55 inch TVs</Link> guide.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=rolling+tv+stand+with+wheels&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
