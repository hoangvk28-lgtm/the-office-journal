...
export const faq = [
  {
    "q": "Will any Wi-Fi 7 router work with my fiber plan?",
    "a": "Only if its WAN port speed matches or exceeds your plan, a gigabit-only WAN port bottlenecks fiber plans faster than 1Gbps regardless of Wi-Fi speed."
  },
  {
    "q": "Do I need a 10G router for a 1Gbps fiber plan?",
    "a": "5G WAN port is already well ahead of a 1Gbps plan, a 10G port matters mainly for plans meaningfully faster than that."
  },
  {
    "q": "Why isn't my device getting full multi-gig speed?",
    "a": "The connected device's own network adapter must also support matching speeds, the router's port alone doesn't guarantee it."
  },
  {
    "q": "Does my ISP require special router configuration for fiber?",
    "a": "Some do, PPPoE or VLAN tagging requirements vary by ISP and not every router supports them by default, verify with your provider first."
  }
];

export const guideSlug = "best-wifi-7-routers-for-fiber-internet";

export const guideTitle = "The Best Wi-Fi 7 Routers for Fiber Internet: Picks and Trade-offs";
...
export const howWeEvaluated = [
  {
    "title": "WAN port speed checked against realistic fiber plan tiers",
    "description": "Verified each router's WAN port speed against common fiber plan tiers to flag where a gigabit-only port would bottleneck faster plans."
  },
  {
    "title": "",
    "description": "Noted that achieving genuine multi-gig throughput also requires the connected device's own network adapter to support matching speeds."
  },
  {
    "title": "CPU-governed routing throughput ceiling considered, not just port spec",
    "description": "Flagged that a router's NAT and routing throughput ceiling can bottleneck real speed even with an adequate port."
  },
  {
    "title": "ISP-specific WAN configuration requirements noted",
    "description": "Noted that some fiber ISPs require PPPoE or VLAN tagging configuration that not all routers support out of the box."
  }
];
...
export const introParagraphs = [
  ...
  "Achieving multi-gig throughput also requires your client device's own network adapter to support matching speeds, and a router's NAT and routing throughput ceiling, which is CPU-governed rather than just a port spec, can bottleneck real speed even with an adequate WAN port, plus some fiber ISPs require specific WAN configuration like PPPoE or VLAN tagging that not all routers support out of the box."
];
...
export const metaDescription = "A practical comparison of 4 wi-fi 7 routers for fiber internet, focused on compatibility, build, features and price, and the trade-offs between them.";
...
export const products: GuideProduct[] = [
  {
    ...
    "description": "5G port, appropriate for fiber plans exceeding 1Gbps where a gigabit-only WAN port would otherwise become the real bottleneck.\n\nVerify your specific ISP's fiber plan speed against this router's 10G port capability, and confirm whether your ISP requires PPPoE or VLAN tagging, since not every router supports these configurations out of the box.\n\n320MHz channel width for real wireless headroom on clean spectrum. That said, achieving full multi-gig speed also requires a matching client-device adapter.",
    "specs": [
      "10G port + 2.5G port + 3x 2.5G LAN",
      "Genuine tri-band with 6GHz radio",
      "320MHz channel width support",
      "6-stream, BE9700 combined rating"
    ],
    "pros": [
      "Genuine 10G port matches fiber plans well beyond 1Gbps",
      "320MHz channel width for real wireless headroom on clean spectrum",
      "Full multi-gig LAN port set avoids wired bottlenecks downstream"
    ],
    "cons": [
      "Achieving full multi-gig speed also requires a matching client-device adapter",
      "Verify PPPoE or VLAN tagging support for your specific ISP before buying",
      "Higher price than gigabit-WAN routers for buyers on slower fiber plans"
    ],
  ...
  },
  {
    ...
    "description": "A newer Wi-Fi 7 chipset generation router appropriate for fiber households wanting the latest hardware, though verify its specific WAN port speed against your actual fiber plan before assuming it avoids a bottleneck.\n\nA router's own CPU-governed NAT and routing throughput ceiling can bottleneck real speed even with an adequate port, worth checking real-world routing throughput reports rather than the port spec alone.\n\n3-star rating despite a smaller review sample.",
    "specs": [
      "Genuine tri-band with 6GHz radio",
      "BE12000 combined rating",
      "New Wi-Fi 7 chipset generation"
    ],
    "pros": [
      "Newer chipset generation for potentially better routing throughput",
      "Genuine tri-band 6GHz radio for full Wi-Fi 7 capability",
      "Good option for buyers prioritizing newer hardware"
    ],
    "cons": [
      "Verify WAN port speed explicitly against your specific fiber plan",
      "Higher price than entry tri-band routers for slower fiber plans"
    ],
  ...
  },
  {
    ...
    "description": "2 Gbps, appropriate for buyers on high-tier fiber plans wanting genuine multi-gig WAN capability plus flexibility to configure one port as WAN and one as a fast LAN uplink.\n\nMesh compatible for future whole-home expansion, though confirm your specific ISP allows using owned equipment as sole gateway rather than requiring their own hardware in the path.\n\nMesh compatible for future whole-home expansion.",
    "specs": [
      "Dual 10G ports",
    ...
    "pros": [
      "Dual 10G ports for genuine multi-gig WAN plus fast LAN flexibility",
      "Mesh compatible for future whole-home expansion",
      "Strong fit for high-tier fiber plans"
    ],
    "cons": [
      "Higher price appropriate mainly for buyers on high-tier fiber plans",
      "Confirm your ISP allows owned equipment as sole gateway before buying",
      "Achieving full 10G speed requires a matching client-device adapter too"
    ],
  ...
  },
  {
    ...
    "description": "5G ports give this router the most wired headroom in this guide, appropriate for very high-tier fiber plans and households with multiple wired devices needing simultaneous multi-gig throughput.\n\nA router's own NAT and routing throughput ceiling is CPU-governed, worth checking real-world routing benchmarks for this model rather than assuming the port specs alone guarantee full fiber-speed throughput.\n\n5G ports for multiple simultaneous multi-gig wired devices. 0-star rating, verify recent reviews.",
    "specs": [
      "2x 10G + 4x 2.5G ports",
      "Genuine tri-band with 6GHz radio",
      "12-stream, BE19000 combined rating",
      "Highest port count in this guide"
    ],
    "pros": [
      "2x 10G ports for maximum wired multi-gig headroom",
      "4x 2.5G ports for multiple simultaneous multi-gig wired devices",
      "Strong fit for very high-tier fiber plans",
      "Full matched multi-gig LAN avoids any wired bottleneck"
    ],
    "cons": [
      "Overkill and costly for typical sub-1Gbps fiber plans",
      "Real-world routing throughput still worth verifying against fiber plan speed"
    ],
...
export const breadcrumbLabel = "Best Wi-Fi 7 Routers for Fiber Internet";
