...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/singles_pool.json"), "utf8"));
const poolArrRaw = [];
...
const buckets = {
  clock: bucket("day clock with calendar"),
  nvme: bucket("NVMe SSD enclosure"),
  filebox: bucket("pink portable file box").filter((p) => /pink/i.test(p.title)),
  divider: bucket("room divider"),
  converter: bucket("standing desk converter"),
};

function categorize(filename) {
  const f = filename.toLowerCase();
  if (f.includes("day-clocks")) return buckets.clock;
  if (f.includes("nvme-enclosures")) return buckets.nvme;
  if (f.includes("pink-portable-file-boxes")) return buckets.filebox.length ? buckets.filebox : buckets.clock;
  if (f.includes("room-dividers")) return buckets.divider;
  if (f.includes("standing-desk-converters")) return buckets.converter;
  if (f.includes("time-displays")) return buckets.clock;
...
function themeFromFilename(filename) {
  ...
  s = s
    .replace(/-?day-clocks-with-calendars?-?/g, " ")
    .replace(/-?nvme-enclosures?-?/g, " ")
    .replace(/-?pink-portable-file-boxes?-?/g, " ")
    .replace(/-?room-dividers?-for-home-offices?-?/g, " ")
    .replace(/-?standing-desk-converters?-?/g, " ")
    .replace(/-?time-displays?-?/g, " ")
    .replace(/-/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return s || "a general-purpose desk accessory";
}

const PRODUCT_NOUN_PLURAL = "desk accessories";
const PRODUCT_CATEGORY_USE = "everyday desk and home office use";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  const cons = [];
  const priceVal = price ? parseFloat(price.replace(/[^\d.]/g, "")) : null;
  const underMatch = fileTheme.match(/under (\d+)/);
  const ceiling = underMatch ? Number(underMatch[1]) : null;
  if (!price) cons.push("Price not listed, requiring a direct check on the current Amazon listing");
  else if (ceiling && priceVal && priceVal > ceiling) cons.push(`Currently priced above this guide's original $${ceiling} ceiling, screen-equipped drawing tablets have gotten pricier industry-wide, confirm current price before assuming it still fits this budget tier`);
  ...
  const para3 = `Best for buyers specifically looking for ${fileTheme}, though as with any purchase, confirm current price and stock directly on the Amazon listing before ordering.`;
...
console.log("Singles files with broken slots:", brokenFileSet.size);
