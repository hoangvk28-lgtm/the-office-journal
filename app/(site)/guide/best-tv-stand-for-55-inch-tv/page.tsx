...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-for-55-inch-tv";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-for-55-inch-tv` },
    about: [{"@type":"Thing","name":"TV Stand"},{"@type":"Thing","name":"55 Inch TV"},{"@type":"Thing","name":"Entertainment Center"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stands for 55 Inch TVs", item: `${SITE_URL}/guide/best-tv-stand-for-55-inch-tv` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-for-55-inch-tv#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stands for 55 Inch TVs</span>
        </nav>
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-5 pt-5 border-t border-border">
            ...
            <div className="flex items-center gap-1.5 text-xs text-ink-muted">
              ...
              {products.length} TV stands evaluated
            </div>
          </div>
        </header>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stands for 55 Inch TVs" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Choosing a TV stand for a 55 inch TV comes down to three things competitors rarely explain clearly: whether the top surface actually supports your TV plus a soundbar and streaming gear, whether a corner or floating design fits your specific room layout, and how much enclosed versus open storage you actually need for the equipment behind the screen.</p>
          <p>This guide covers seven 55 inch TV stands ranked by weight capacity, storage style, and real clearance considerations, spanning a budget console under $60, a floating wall-mount option, two corner-fit designs, a modern LED gaming stand, and a fireplace-equipped console.</p>
          <p>For help organizing the cables behind whichever stand you choose, see our <Link prefetch={false} href="/guide/best-cable-management-boxes-for-desks" className="text-brand hover:underline">best cable management boxes for desks</Link> guide.</p>
        </section>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-ink-secondary">
                ...
                {" "}- {products[0]?.badge.toLowerCase()} pick in this roundup, with the highest rated top surface capacity here.
              </p>
            </div>
            <p className="text-sm text-ink-secondary pl-5">See the full rankings above for all seven picks with pros, cons, and best-for summaries.</p>
          </div>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+for+55+inch+tv&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
