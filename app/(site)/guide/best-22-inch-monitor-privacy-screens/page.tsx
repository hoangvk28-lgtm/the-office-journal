...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-22-inch-monitor-privacy-screens";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-22-inch-monitor-privacy-screens",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-22-inch-monitor-privacy-screens` },
    about: [{"@type":"Thing","name":"Best 22-Inch Monitor Privacy Screens"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 22-Inch Monitor Privacy Screens", item: `${SITE_URL}/guide/best-22-inch-monitor-privacy-screens` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-22-inch-monitor-privacy-screens#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 22-Inch Monitor Privacy Screens</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-white h-56 sm:h-64 md:hidden flex items-center justify-center p-3">
          <Image src={heroImg} alt="Best 22-Inch Monitor Privacy Screens" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
        </div>
        <div
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>22 inch is one of the sizes where aspect ratio genuinely varies: some 22 inch monitors are 16:9, others are 16:10, and the two are not interchangeable for filter fit. We confirmed the exact ratio for every pick here.</p>
          <p>This size tier has a smaller confirmed product pool than the more common 24 and 27 inch sizes in our research, so we're including both dedicated 22 inch picks and confirmed-compatible nearby options.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 22-Inch Monitor Privacy Screen</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 22-Inch Monitor Privacy Screens</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 22-Inch Monitor Privacy Screens</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=22+inch+monitor+privacy+screen&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
