...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-cable-management-boxes-for-desks";
...
export default async function BestCableManagementBoxesForDesksPage() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-cable-management-boxes-for-desks` },
    about: [
        {
          "@type": "Thing",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Cable Management Boxes for Desks", item: `${SITE_URL}/guide/best-cable-management-boxes-for-desks` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-cable-management-boxes-for-desks#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Cable Management Boxes for Desks</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Cable Management Boxes for Desks" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Cable management boxes hide power strips, surge protectors, and cable clusters from view, turning the tangled wire pile behind a desk into a clean organized unit. They work on desk surfaces, floors, and under desk surfaces via clamp-mount trays.</p>
          <p>This guide covers eight cable management boxes for desks ranked by size, design, safety features, and value, including on-desk boxes and under-desk clamp trays.</p>
          <p>For wood-aesthetic cable management options, see our <a href='/guide/best-wood-cable-management-boxes' className='text-brand hover:underline'>best wood cable management boxes</a> guide.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href="https://www.amazon.com/s?k=cable+management+box+for+desk&tag=theofficejournal-20" target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
