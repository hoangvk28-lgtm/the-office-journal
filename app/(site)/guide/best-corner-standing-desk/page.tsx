...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-corner-standing-desk";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-corner-standing-desk` },
    about: [{"@type":"Thing","name":"Corner Standing Desk"},{"@type":"Thing","name":"L-Shaped Standing Desk"},{"@type":"Thing","name":"Electric Height Adjustable Desk"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Corner Standing Desks", item: `${SITE_URL}/guide/best-corner-standing-desk` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-corner-standing-desk#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Corner Standing Desks</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Corner Standing Desks" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A corner standing desk maximizes your available wall space by using two walls simultaneously, giving you more surface area than a straight desk of the same listed width.</p>
          <p>This guide covers eight L-shaped electric standing desks ranked by frame stability, motor configuration, desktop size, and price, from budget options under $200 to heavy-duty triple-motor frames.</p>
          <p>For a straight standing desk instead of an L-shape, see our <a href='/guide/best-standing-desk-for-dual-monitors' className='text-brand hover:underline'>best standing desks for dual monitors</a> guide.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=corner+L-shaped+standing+desk&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
