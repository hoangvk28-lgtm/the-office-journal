...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/pps_pool.json"), "utf8"));
const poolArrRaw = [];
...
function categorize(filename) {
  ...
  if (f.includes("under-2000")) return buckets.twoK;
  return buckets.fiveK.length ? buckets.fiveK : buckets.twoK;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?portable-power-stations?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose portable power station";
}

const PRODUCT_NOUN_PLURAL = "portable power stations";
const PRODUCT_CATEGORY_USE = "home backup power and off-grid use";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  const para3 = `Best for buyers specifically looking for ${fileTheme}, though as with any backup power purchase, confirm current price and stock directly on the Amazon listing before ordering.`;
...
console.log("Portable power station files with broken slots:", brokenFileSet.size);
