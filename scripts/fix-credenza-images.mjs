...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/credenza_pool.json"), "utf8"));
const poolArrRaw = [];
...
const isRealCredenza = (p) => /credenza|cabinet|buffet|sideboard|file cabinet/i.test(p.title);
const poolArr = Object.values(byAsin).filter(isRealCredenza);

function bucket(tagSubstr) {
  return poolArr.filter((p) => p.tags.some((t) => t.includes(tagSubstr)));
}
const buckets = {
  sixty: bucket("60 inch office credenza"),
  hutch: bucket("office credenza with hutch"),
  modern: bucket("modern office credenza"),
  budget: bucket("budget office credenza"),
};
...
function categorize(filename) {
  ...
  if (f.includes("60-inch")) return buckets.sixty.length ? buckets.sixty : buckets.modern;
  if (f.includes("under-700")) return under(700).length ? under(700) : buckets.budget;
  if (f.includes("under-1000")) return under(1000).length ? under(1000) : buckets.modern;
  if (f.includes("under-1500")) return under(1500).length ? under(1500) : buckets.hutch;
  return buckets.modern.length ? buckets.modern : buckets.budget;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?office-credenzas?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose office credenza";
}

const PRODUCT_NOUN_PLURAL = "office credenzas";
const PRODUCT_CATEGORY_USE = "daily home office storage and organization";
...
console.log("Office credenza files with broken slots:", brokenFileSet.size);
