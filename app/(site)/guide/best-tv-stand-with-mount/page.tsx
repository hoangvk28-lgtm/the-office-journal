...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-with-mount";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-with-mount` },
    about: [{"@type":"Thing","name":"TV Stand with Mount"},{"@type":"Thing","name":"TV Console with Bracket"},{"@type":"Thing","name":"Swivel TV Mount Stand"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TV Stand with Mount", item: `${SITE_URL}/guide/best-tv-stand-with-mount` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-with-mount#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TV Stand with Mount</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best TV Stands with Mount" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A TV stand with mount builds the wall-mount look into the furniture itself: a steel bracket or pole on the back panel holds the TV a few inches above the console top, so the screen appears to float without any wall drilling.</p>
          <p>This guide covers eight built-in mount consoles ranked by VESA compatibility, mount weight rating, swivel and tilt range, and storage, from budget corner units under $60 to furniture-grade cabinets with power outlets and LED lighting.</p>
          <p>If your TV is already mounted separately and you only need media storage beneath it, see our <a href='/guide/best-tv-stand-for-55-inch-tv' className='text-brand hover:underline'>best TV stands for 55 inch TVs</a> guide instead.</p>
        </section>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-ink-secondary">
                ...
                {" "}- {products[0]?.badge.toLowerCase()} pick in this roundup with the widest VESA range and highest mount weight rating.
              </p>
            </div>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+with+mount&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
