...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-room-divider-bookcase";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-room-divider-bookcase",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-room-divider-bookcase` },
    about: [{"@type":"Thing","name":"Best Room Divider Bookcases"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Room Divider Bookcases", item: `${SITE_URL}/guide/best-room-divider-bookcase` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-room-divider-bookcase#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Room Divider Bookcases</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Room Divider Bookcases" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A room divider bookcase has to earn its keep from both sides. Unlike a bookshelf pushed against a wall, it sits in the open, so its back looks almost as important as its front, and it needs to be stable enough to stand freely without tipping.</p>
          <p>Below are 7 room divider bookcases we evaluated on width, total capacity, and how well they hold up as a genuine freestanding divider, ranging from a slim S-shaped geometric rack to a 70-inch-wide etagere built for splitting a large open-concept room.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=room+divider+bookcase&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
