...
if (!cluster) throw new Error("usage: node fetch-generic.mjs <cluster>");
...
const allAsins = JSON.parse(readFileSync(resolve(ROOT, "scripts", `unique_asins_${cluster}.txt`), "utf8"));
...
writeFileSync(resolve(ROOT, "scripts", `data-${cluster}.json`), JSON.stringify(allItems, null, 2), "utf8");
console.log(`Saved to scripts/data-${cluster}.json`);
