...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/dtab_pool.json"), "utf8"));
const poolArrRaw = [];
...
const isRealScreenTablet = (p) => /drawing (tablet|monitor).*screen|pen display/i.test(p.title) && !/lcd writing tablet/i.test(p.title);
const poolArr = Object.values(byAsin).filter(isRealScreenTablet).sort((a, b) => {
  const pa = parseFloat((a.price||"999999").replace(/[^\d.]/g,"")) || 999999;
  const pb = parseFloat((b.price||"999999").replace(/[^\d.]/g,"")) || 999999;
  return pa - pb;
});

function categorize(filename) {
  return poolArr; // cheapest-first pool, only 3 files, pick lowest-price-first per file
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?drawing-tablets-with-screens?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose drawing tablet with screen";
}

const PRODUCT_NOUN_PLURAL = "drawing tablets with screens";
const PRODUCT_CATEGORY_USE = "digital art and illustration work at a desk";
...
function buildContent(product, fileTheme, rank, badge, idBase) {
  ...
  const para3 = `Best for buyers specifically looking for ${fileTheme}, though as with any tablet purchase, confirm current price and stock directly on the Amazon listing before ordering.`;
...
console.log("Drawing tablet files with broken slots:", brokenFileSet.size);
