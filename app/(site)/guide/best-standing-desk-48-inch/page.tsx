...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-standing-desk-48-inch";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-standing-desk-48-inch` },
    about: [{"@type":"Thing","name":"48 Inch Standing Desk"},{"@type":"Thing","name":"Compact Standing Desk"},{"@type":"Thing","name":"Electric Height Adjustable Desk"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 48 Inch Standing Desks", item: `${SITE_URL}/guide/best-standing-desk-48-inch` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-standing-desk-48-inch#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 48 Inch Standing Desks</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best 48 Inch Standing Desks" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A 48-inch standing desk is the most popular compact size, fitting comfortably in most bedrooms, studios, and small home offices while handling a single monitor or laptop plus peripherals.</p>
          <p>This guide covers eight 48-inch electric standing desks ranked by price, special features (drawer, power strip, USB ports), depth (24 vs 30 inch), and brand warranty.</p>
          <p>For a wider surface in the same price range, see our <a href='/guide/best-standing-desk-for-dual-monitors' className='text-brand hover:underline'>best standing desks for dual monitors</a> guide (55 to 72 inch options).</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=48+inch+standing+desk+electric&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
