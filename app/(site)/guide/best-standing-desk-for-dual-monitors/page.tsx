...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-standing-desk-for-dual-monitors";
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-standing-desk-for-dual-monitors` },
    about: [{"@type":"Thing","name":"Standing Desk"},{"@type":"Thing","name":"Dual Monitor Desk"},{"@type":"Thing","name":"Wide Electric Desk"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Standing Desks for Dual Monitors", item: `${SITE_URL}/guide/best-standing-desk-for-dual-monitors` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-standing-desk-for-dual-monitors#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Standing Desks for Dual Monitors</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Standing Desks for Dual Monitors" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A standing desk for dual monitors needs at least 55 inches of surface width, a stable frame that handles the combined monitor weight, and a height range that covers both your sitting and standing positions.</p>
          <p>This guide covers eight standing desks for dual monitors ranked by width (48 to 72 inches), surface depth, motor configuration, and special features like bamboo tops, risers, and drawers.</p>
          <p>For the widest possible surface in an L-shaped format, see our <a href='/guide/best-corner-standing-desk' className='text-brand hover:underline'>best corner standing desks</a> guide.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=standing+desk+dual+monitor+wide&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
