...
import {
  ...
  buyingCriteria,
  type GuideProduct,
} from "@/data/guides/best-tv-stand-corner";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-tv-stand-corner",
...
export default async function Page() {
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tv-stand-corner` },
    about: [{"@type":"Thing","name":"Best Corner TV Stands (Floor Consoles + Wall Mounts)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Corner TV Stands (Floor Consoles + Wall Mounts)", item: `${SITE_URL}/guide/best-tv-stand-corner` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tv-stand-corner#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Corner TV Stands (Floor Consoles + Wall Mounts)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          <Image src={heroImg} alt="Best Corner TV Stands (Floor Consoles + Wall Mounts)" width={900} height={420} className="w-full h-auto object-contain" priority unoptimized />
        </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A "corner TV stand" search actually covers two different products: a floor console shaped with a 90-degree back to sit flush in a room corner, and a wall-mounted bracket with an extension arm long enough to pull the TV out past the corner's apex. Buying the wrong type for your room means either a console that never fits flat against both walls or a wall mount that leaves the screen crammed into the angle instead of centered on your seating area.</p>
          <p>This guide ranks both categories together so you can match the product to your actual corner, whether that means freeing up floor space with a bracket and a long arm or anchoring media storage underneath the TV with a shaped console.</p>
        </section>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=tv+stand+corner&tag=theofficejournal-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
