...
const pool = JSON.parse(readFileSync(resolve(ROOT, "/tmp/reception_pool.json"), "utf8"));
const poolArrRaw = [];
...
// keep only genuine reception desk / counter furniture, exclude podiums, decor, LED sheets, raw crystal, etc.
const isRealDesk = (p) =>
  /desk|counter/i.test(p.title) && !/podium|pulpit|lectern|crystal|backlight|led panel|countertop slab|coffee table top/i.test(p.title);
const poolArr = Object.values(byAsin).filter(isRealDesk);

function bucket(tagSubstr) {
  return poolArr.filter((p) => p.tags.some((t) => t.includes(tagSubstr)));
}
const buckets = {
  large: bucket("large reception"),
  small: bucket("small reception"),
  mobile: bucket("wheels mobile"),
  portable: bucket("portable folding"),
  stone: bucket("stone quartz"),
  lshaped: bucket("L-shaped reception"),
  modern: bucket("modern reception"),
  budget: bucket("budget reception"),
  storage: bucket("storage cabinet"),
};

function categorize(filename) {
  const f = filename.toLowerCase();
  if (f.includes("concrete") || f.includes("quartz")) return buckets.stone.length ? buckets.stone : buckets.large;
  if (f.includes("mobile") || f.includes("on-wheels")) return buckets.mobile.length ? buckets.mobile : buckets.portable;
  if (f.includes("portable")) return buckets.portable.length ? buckets.portable : buckets.mobile;
  if (f.includes("under-300")) return buckets.budget;
  if (f.includes("under-450") || f.includes("under-700") || f.includes("under-800")) return [...buckets.budget, ...buckets.modern];
  if (f.includes("10ft") || f.includes("96-inch") || f.includes("9ft")) return buckets.large;
  if (f.includes("6ft") || f.includes("8ft") || f.includes("90-inch")) return [...buckets.lshaped, ...buckets.modern];
  return buckets.modern.length ? buckets.modern : buckets.large;
}
...
function themeFromFilename(filename) {
  ...
  s = s.replace(/-?reception-desks?-?/g, " ").replace(/-/g, " ").replace(/\s+/g, " ").trim();
  return s || "a general-purpose reception desk";
}

const PRODUCT_NOUN_PLURAL = "reception desks";
const PRODUCT_CATEGORY_USE = "daily front-desk and visitor-facing use";
...
console.log("Reception desk files with broken slots:", brokenFileSet.size);
